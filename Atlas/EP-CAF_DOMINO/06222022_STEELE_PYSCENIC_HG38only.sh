#!/bin/bash
#SBATCH --job-name=STEELE_PYSCENIC_HG38only
...
RESULT_DIR="PDAC_EpCAF/STEELE_HG38only"
COUNTS_LOOM="PDAC_EpCAF/cds_Steele_EpCAF.loom"

# run step1
singularity exec "${DOMINO_DIR}/aertslab-pyscenic-0.11.0.sif" pyscenic grn \
	$COUNTS_LOOM \
	"${DOMINO_DIR}/allTFs_hg38.txt" \
	-o "${RESULT_DIR}/adj_PDAC_Steele_EpCAF_hg38.tsv" \
	--num_workers 10
...
singularity exec "${DOMINO_DIR}/aertslab-pyscenic-0.11.0.sif" pyscenic ctx \
	"${RESULT_DIR}/adj_PDAC_Steele_EpCAF_hg38.tsv" \
	...
	--output "${RESULT_DIR}/regulons_PDAC_Steele_EpCAF_hg38.csv" \
	--num_workers 10
...
singularity exec "${DOMINO_DIR}/aertslab-pyscenic-0.11.0.sif" pyscenic aucell \
	"${COUNTS_LOOM}" \
	"${RESULT_DIR}/regulons_PDAC_Steele_EpCAF_hg38.csv" \
	-o "${RESULT_DIR}/auc_PDAC_Steele_EpCAF_hg38.csv"
