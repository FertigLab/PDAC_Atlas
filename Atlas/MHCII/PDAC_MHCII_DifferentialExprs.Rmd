---
title: "PDAC_MHCII_DifferentialExprs"
author: "Joe"
date: "4/21/2022"
...
##Load CDS from Ben
```{r}
PDAC_cds <- readRDS("cds_duct_SHARED_GENES_QC_filtered_harmonized_preprocessed_aligned_manuscript_UMAP_learn_graph.rds")
```
...
##Save fitted models
saveRDS(gene_fits, file="DuctalDE_forTvsN.rds")
...
saveRDS(fit_coefs, file="fit_coefs.rds")
...
saveRDS(MHCII_table, file="MHCII_table.rds")
saveRDS(MHCII_table_subset, file="MHCII_table_subset.rds")

##Save DE results for MHCII Genes
MHCII_table_subset_saved <- MHCII_table_subset %>%
         dplyr::select(gene_short_name, term, p_value, q_value, estimate)
write.csv(MHCII_table_subset_saved, file="MHCII_table_subset.csv")
...
##Load Atlas
PDAC_cds <- readRDS("cds_duct_SHARED_GENES_QC_filtered_harmonized_preprocessed_aligned_manuscript_UMAP_learn_graph.rds")
rownames(PDAC_cds) <- rowData(PDAC_cds)$gene_short_name

##Subset Atlas for MHCII Genes
cds_subset <- PDAC_cds[rownames(PDAC_cds) %in% MHCII_genes,]

##Make Figures of results
pdf(file = "Duct_MHCII_DE_wholegenome.pdf")
